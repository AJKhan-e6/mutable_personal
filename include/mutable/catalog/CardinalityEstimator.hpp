...
struct M_EXPORT SpnEstimator : CardinalityEstimatorCRTP<SpnEstimator>
{
    ...
    // std::unordered_map<const char*, std::vector<Spn::LeafType>> leafTypesMap_;

    public:
        explicit SpnEstimator(const char *name_of_database) : name_of_database_(name_of_database) { }
    // explicit SpnEstimator(const char *name_of_database,  std::unordered_map<const char*, std::vector<Spn::LeafType>> leafTypesMap) : name_of_database_(name_of_database), leafTypesMap_(leafTypesMap) { }
