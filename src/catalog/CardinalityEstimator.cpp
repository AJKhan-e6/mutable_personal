...
void SpnEstimator::learn_spns() { 
    ...
    table_to_spn_ = SpnWrapper::learn_spn_database(name_of_database_);
    }

void SpnEstimator::learn_new_spn(const char *name_of_table)
{
    std::cout << "Entered learn_new_spn" << std::endl;
    ...
    for (const auto& entry : table_to_spn_) {
    const char* tableName = entry.first;
    SpnWrapper* spnWrapper = entry.second;

    // Print the table name and associated SpnWrapper
    std::cout << "Table Name: " << tableName << std::endl;
    // You might want to print details of the SpnWrapper here if needed.
    }
...
std::unique_ptr<DataModel> SpnEstimator::estimate_scan(const QueryGraph &G, Subproblem P) const
{
    M_insist(P.size() == 1);
    const auto idx = *P.begin();
    std::cout << "The number we got out of Subproblem P is: " << idx << std::endl;
    auto &BT = as<const BaseTable>(*G.sources()[idx]);
...
std::unique_ptr<DataModel> SpnEstimator::estimate_filter(const QueryGraph&, const DataModel &_data, const cnf::CNF &filter) const
{
    ...
    for (const auto& pair : attribute_to_id) {
    ...
    std::cout << "Entered the filter estimator" << std::endl;
    ...
    for (auto &clause : filter) {
        ...
        std::cout << "ft.attribute is: " << ft.attribute << std::endl;
                            ...
                            const cnf::CNF &condition) const
{
    ...
    std::cout << "Etnered the estimate_join function" << std::endl;
    ...
    if (not condition.empty()) {
        ...
        jt(*condition[0][0]);
        std::cout << "*condition [0][0] is: " << condition[0][0] << std::endl;
