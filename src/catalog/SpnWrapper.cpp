...
// std::unordered_map<const char*, std::shared_ptr<SpnWrapper>> SpnWrapper::learn_spn_database(const char *name_of_database,
//                                std::unordered_map<const char*, std::vector<Spn::LeafType>> leaf_types)
// {
//     auto &C = Catalog::Get();
//     auto &db = C.get_database(C.pool(name_of_database));

//     std::unordered_map<const char*, std::shared_ptr<SpnWrapper>> spns;

//     for (auto table_it = db.begin_tables(); table_it != db.end_tables(); table_it++) {
//         spns.emplace(
//             table_it->first,
//             std::make_shared<SpnWrapper>(
...
SpnWrapper::learn_spn_database(const char *name_of_database,
                               std::unordered_map<const char*, std::vector<Spn::LeafType>> leaf_types)
{
    std::cout << "Entered the learn_spn_database method" << std::endl;

    auto &C = Catalog::Get();
    std::cout << "Catalog size is: " << C.num_databases() << std::endl;
    std::cout << "Whether databse is in use: " << C.has_database_in_use() << std::endl;
    std::cout << "Name of database is: " << name_of_database << std::endl;
    std::cout << "What c.pool is returning: " << C.pool(name_of_database) << std::endl;
    C.list_database();
    ...
    for (auto table_it = db.begin_tables(); table_it != db.end_tables(); table_it++) {
        ...
        std::cout << "table is: " << table_it->first << std::endl;
    ...
    std::cout << "learnt the spns" << std::endl;
